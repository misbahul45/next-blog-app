...
import { getSinglePost } from '@/fetch/posts'
...
const page = async( { params:{ slug } }:Params ) => {
  ...
  return (
    <section className={`py-4 w-full max-w-[90%] mx-auto ${ubuntu.className}`}>
      <h1 className='text-3xl text-blue-900 text-center font-bold'>{title}</h1> 
      <PostAuthor createdAt={UpdatedAt} authorId={authorId} /> 
      <div className="relative min-h-96 w-full shadow-lg shadow-slate-600 rounded-lg my-4">
        {image?
          <Image alt={title} src={image} fill sizes='100%' className='object-cover rounded-lg' />
          :
          <div className="w-full h-96 bg-slate-800 flex justify-center items-center">
            <IoImage className='h-full w-full text-slate-100 opacity-75' />
          </div>
        }
      </div>
